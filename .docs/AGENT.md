...
6. **Siga Estritamente o Guia de Contribuição:** Siga rigorosamente as regras definidas em [CONTRIBUTING.md](../CONTRIBUTING.md). Use o fluxo de GitFlow (criar branches específicas a partir de `feat/`, `fix/` ou `refactor/` a partir de `main` atualizada), nunca faça commits diretos na `main`, valide o build local com `npm run build` e preencha o template de Pull Request correspondente. Os agentes devem dar o exemplo de conformidade para novos contribuidores.
...
* **Ao Finalizar a Tarefa:** Antes de propor alterações ou abrir o Pull Request, certifique-se de preencher o template contido em `.github/pull_request_template.md` e validar que as regras de segurança e build local do [CONTRIBUTING.md](../CONTRIBUTING.md) foram cumpridas.
